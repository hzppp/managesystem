    ...
    <el-col :span="8">
      <el-card shadow="hover">
        ...
        <div class="login-info">
          ...
          <p>上次登录地点：<span>GuangZhou</span></p>
        </div>
      </el-card>
  ...
  methods: {
    getTableData() {
      this.$http.get("/home/getData").then((res) => {
        ...
        this.echartData.order.xData = order.date;//获取到xData x轴数据
        let keyArray = Object.keys(order.data[0]);//返回键名
        // 然后根据键名，循环对series添加数据
        keyArray.forEach(key => {
          this.echartData.order.series.push({
            name : key === 'wechat' ? '小程序' : key,
            data : order.data.map(item => item[key]), //根据键名返回值
            type : 'line'
          })
        })
